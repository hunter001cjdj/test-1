...
<form action="search-beitou-output.php" method="post">
...
$sql = $pdo->prepare('SELECT * FROM beitou WHERE kind LIKE ? OR name LIKE ? OR price LIKE ? OR address LIKE ? OR hours LIKE ?');
// 執行查詢，使用關鍵字
...
echo '<td><a href="../shop-details/beitou-details.php?id=' . htmlspecialchars($row['id']) . '">' . htmlspecialchars($row['name']) . '</a></td>';
